...
- **發行管線（v0.22.0，1.0 交付化）**：`node scripts/release.mjs <x.y.z>` 升版號＋CHANGELOG 段落（自上個 tag 的 commit 標題）＋commit＋tag＋push → `.github/workflows/release.yml` 建 **GHCR 單體映像** `ghcr.io/rogs30541/sitekit:<版號>|latest` 與 **免建置發行包** `sitekit-<版號>.tar.gz`（含 node_modules 與 web 建置；解壓後 `node apps/server/bin/sitekit.mjs start` 即開站，適合 VPS／cPanel Node 主機）並開 GitHub Release。`deploy/zeabur-template.yaml`＝Zeabur 一鍵模板（單體殼＋PostgreSQL，網域填一次）。後台頂部會顯示「有新版本」（api `GET /api/admin/system/update-check`，查 GitHub Releases latest，6 小時快取；`SITEKIT_UPDATE_REPO` 可改、空字串關閉）。**原始碼 repo 公開前，GHCR 映像與 Releases API 都是私有**：模板拉映像要加 registry 認證、更新檢查會靜默失敗。
