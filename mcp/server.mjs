...
server.tool('sitekit_recommend_site_template', '依行業／風格／關鍵字推薦版型（不改資料）', { category: z.enum(['image', 'shop', 'course', 'brand', 'service']).optional(), industry: z.string().optional(), style: z.string().optional(), keywords: z.array(z.string()).optional() }, async (p) => asText(await ops('recommend_site_template', p)));
server.tool('sitekit_quick_setup_site', '一鍵建站（confirm 必須 true）：挑版型並套用、把品牌名／標語／Email／電話寫進區塊、更新品牌設定與主題；不產圖不杜撰文案', { confirm: z.boolean().default(false), templateId: z.string().optional(), category: z.enum(['image', 'shop', 'course', 'brand', 'service']).optional(), industry: z.string().optional(), style: z.string().optional(), keywords: z.array(z.string()).optional(), brandName: z.string().optional(), siteName: z.string().optional(), tagline: z.string().optional(), description: z.string().optional(), contactEmail: z.string().optional(), phone: z.string().optional(), address: z.string().optional(), accent: z.string().optional(), mode: z.enum(['light', 'dark']).optional() }, async (p) => asText(await ops('quick_setup_site', p)));
...
server.tool('sitekit_draft_contact_reply', 'AI 擬回覆草稿（不寄信；只用原訊息＋要點，不知道的事實留【請補充】）；審閱後用 sitekit_reply_contact_message 寄出', { id: z.string(), tone: z.string().optional(), points: z.string().optional() }, async (p) => asText(await ops('draft_contact_reply', p)));
server.tool('sitekit_list_ai_conversations', '列出後台 AI 指令台對話記錄摘要（標題／回合數／管理員）', { limit: z.number().optional() }, async (p) => asText(await ops('list_ai_conversations', p)));
...
server.tool('sitekit_list_sales_templates', '列出一頁式網頁套版（五分類 × 5 風格配色 ＝ 25 套）', { category: z.enum(['image', 'shop', 'course', 'brand', 'service']).optional() }, async (p) => asText(await ops('list_sales_templates', p)));
server.tool('sitekit_apply_sales_template', '用套版建立／覆寫銷售頁草稿（confirm 必須 true；不發佈）', { id: z.string(), slug: z.string().optional(), title: z.string().optional(), code: z.string().optional(), confirm: z.boolean().default(false) }, async (p) => asText(await ops('apply_sales_template', p)));
server.tool(
