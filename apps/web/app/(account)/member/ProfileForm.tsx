...
export function ProfileForm({ displayName }: { displayName: string }) {
  ...
  async function changePassword() {
    ...
    setMsg(r.ok ? '密碼已更新' : typeof j.message === 'string' ? j.message : '更新失敗');
    if (r.ok) {
      setCur('');
      setNext('');
    }
  ...
  return (
    <div className="mt-4 space-y-2 text-xs">
      <div className="flex flex-wrap items-center gap-2">
        <input className={input} style={{ borderColor: 'var(--line)' }} value={name} onChange={(e) => setName(e.target.value)} placeholder="顯示名稱" />
        <button onClick={saveName} className="rounded border px-3 py-1" style={{ borderColor: 'var(--line)' }}>
          更新名稱
        </button>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <input className={input} style={{ borderColor: 'var(--line)' }} type="password" autoComplete="current-password" value={cur} onChange={(e) => setCur(e.target.value)} placeholder="目前密碼（第三方登入者可留空）" />
        <input className={input} style={{ borderColor: 'var(--line)' }} type="password" autoComplete="new-password" value={next} onChange={(e) => setNext(e.target.value)} placeholder="新密碼（≥8）" />
