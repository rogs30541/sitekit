    ...
    if (r.status === 401) {
      return (
        <Section title="預覽連結無效或已過期" group="(marketing)">
          <p>請回後台「頁面設計」重新產生沙盒預覽連結（每個連結 2 小時有效）。</p>
        </Section>
      );
    }
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900">
        <span>
          <strong>沙盒預覽</strong>：這是「草稿」內容，尚未發佈，線上頁面不受影響。草稿更新於 {new Date(doc.updatedAt).toLocaleString('zh-TW')}
          {doc.liveStatus === 'published' ? `；線上目前為第 ${doc.liveVersion} 版` : '；此頁尚未上線'}
        </span>
