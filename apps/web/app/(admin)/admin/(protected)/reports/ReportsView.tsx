...
export async function ReportsView({ scope, sp }: { scope: 'shop' | 'course'; sp: { from?: string; to?: string; groupBy?: string } }) {
  const qs = new URLSearchParams({ ...(Object.fromEntries(Object.entries(sp).filter(([, v]) => v)) as Record<string, string>), scope }).toString();
  ...
  return (
    <Section title={scope === 'course' ? '課程報表' : '電商報表'} group="(admin)">
