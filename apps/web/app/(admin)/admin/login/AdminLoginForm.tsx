...
/**
 * 後台登入／註冊／忘記密碼：註冊走 Email 驗證碼（第一位管理員、主管理員 Email 或白名單 email 才收得到驗證碼；第一位自動成為 superadmin）；
 * 忘記密碼寄驗證碼給既有管理員。填錯 Email 收不到信：註冊第 2 步可「更換 Email」重寄；已建立的帳號在後台「管理員帳號 → 我的帳號」改，
 * 或用環境變數 SITEKIT_ADMIN_EMAIL／SITEKIT_ADMIN_PASSWORD、CLI `sitekit admin set-email` 救援（不需收信）。
 */
export function AdminLoginForm() {
  ...
  const [mode, setMode] = useState<'login' | 'register' | 'forgot'>(search.get('mode') === 'forgot' ? 'forgot' : 'login');
  ...
  async function requestCode(e: React.FormEvent) {
    ...
    try {
      ...
      setStep(2);
      setInfo(j.sent === false ? '若此 Email 具備管理員註冊資格，驗證碼已寄出（已有管理員的站台只寄給主管理員 Email 或白名單）。' : `驗證碼已寄到 ${email}（15 分鐘內有效）${j.mail && j.mail.ok === false ? `；寄信失敗：${j.mail.error ?? j.mail.provider}` : j.mail?.provider === 'log' ? '；Email 尚未設定寄信服務，驗證碼只會出現在伺服器日誌' : ''}${j.devCode ? `，開發環境驗證碼：${j.devCode}` : ''}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
      setBusy(false);
  ...
  async function forgot(e: React.FormEvent) {
    ...
    setInfo('');
    try {
      const j = await post('/api/admin/auth/forgot', { email });
      setStep(2);
      setInfo(j.sent === false ? '若此 Email 是管理員，驗證碼已寄出。' : `驗證碼已寄到 ${email}（15 分鐘內有效）${j.mail && j.mail.ok === false ? `；寄信失敗：${j.mail.error ?? j.mail.provider}` : ''}${j.devCode ? `，開發環境驗證碼：${j.devCode}` : ''}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
      setBusy(false);
  ...
  async function reset(e: React.FormEvent) {
    ...
    setError('');
    try {
      await post('/api/admin/auth/reset', { email, code, password });
      go();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
  ...
  return (
    <div className="mt-4">
      <div className="mb-3 flex gap-3 text-sm">
        ...
        <button type="button" onClick={() => (setMode('forgot'), setStep(1), setError(''), setInfo(''))} className={mode === 'forgot' ? 'font-bold underline' : ''} data-mode-forgot>
          忘記密碼
        </button>
      </div>
      {mode === 'login' ? (
        ...
        step === 1 ? (
          <form onSubmit={forgot} className="space-y-3">
            <p className="rounded bg-amber-50 p-2 text-xs text-amber-900">輸入管理員 Email，我們會寄 6 碼驗證碼讓你設定新密碼。收不到信（Email 填錯或尚未設定寄信服務）時，請用環境變數 SITEKIT_ADMIN_EMAIL／SITEKIT_ADMIN_PASSWORD 或 CLI「sitekit admin」救援。</p>
          ...
          <form onSubmit={reset} className="space-y-3">
            ...
            <p className="text-xs">
              {email}{' '}
              <button type="button" onClick={() => (setStep(1), setInfo(''))} className="underline">
                更換 Email
              </button>
            </p>
            ...
            <input className={input} style={{ borderColor: 'var(--line)' }} type="password" required minLength={8} autoComplete="new-password" placeholder="新密碼（至少 8 碼）" value={password} onChange={(e) => setPassword(e.target.value)} />
            <button disabled={busy} className="w-full rounded bg-black px-3 py-2 text-sm text-white disabled:opacity-50">
              {busy ? '重設中…' : '重設密碼並登入'}
            </button>
          </form>
        ...
        <form onSubmit={requestCode} className="space-y-3">
          <p className="rounded bg-amber-50 p-2 text-xs text-amber-900">{needsBootstrap ? '尚未建立任何管理員：完成 Email 驗證後，此帳號將成為第一位超級管理員，這個 Email 也會成為站台的主管理員 Email。' : '站台已有管理員：只有主管理員 Email 或超級管理員設定的白名單 Email（或網域）能完成註冊；驗證碼會寄到你的信箱。已有帳號請直接登入或用「忘記密碼」。'}</p>
          <input className={input} style={{ borderColor: 'var(--line)' }} type="email" required placeholder="管理員 Email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <button disabled={busy} className="w-full rounded bg-black px-3 py-2 text-sm text-white disabled:opacity-50">
            {busy ? '寄送中…' : '寄送驗證碼'}
          </button>
        ...
        <form onSubmit={confirm} className="space-y-3">
          ...
          <p className="text-xs">
            {email}{' '}
            <button type="button" onClick={() => (setStep(1), setInfo(''))} className="underline" data-change-email>
              填錯了？更換 Email 重寄
            </button>
          </p>
          <input className={input} style={{ borderColor: 'var(--line)' }} inputMode="numeric" pattern="\d{6}" required placeholder="6 碼驗證碼" value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, '').slice(0, 6))} />
