...
export class InvoiceService {
  ...
  async config() {
    ...
    const [provider, timing, ez, ezTest, ecId, ecKey, ecIv, ecTest] = await Promise.all([
    ...
    return {
      provider: eff as 'none' | 'ezpay' | 'ecpay',
  ...
  async issue(order: OrderForInvoice, actor: string) {
    ...
    const r = cfg.provider === 'ecpay' ? await this.issueEcpay(order, req, cfg.ecpay) : await this.issueEzpay(order, req, cfg.ezpay);
