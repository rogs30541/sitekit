...
| **D3 追蹤與 SEO** `tracking` | `get_tracking`（R）→ `set_tracking`（W，整份回寫；含 scroll 深度百分比事件）；區塊可視事件在 `set_home_sections`／`upsert_content` 的區塊 `track`；`update_brand`（W：描述／OG 圖／語言） | 「裝 GA4 G-XXXX 和 Meta Pixel」→ 先 `get_tracking` 取現值再整份回傳（未給欄位會被清空）。 |
