...
const TOOL_HINTS: Partial<Record<OpsAction, string>> = {
  ...
  set_tracking: '參數：scroll{enabled,percents[25,50,75,100],event}（頁面滑動深度自訂事件）、ga4、gtm、fbPixel、tiktok、lineTag、googleAdsId、googleAdsLabel、head、bodyTop、bodyBottom、events{pageView,viewContent,addToCart,initiateCheckout,purchase,scroll}（未給的欄位會被清空，先 get_tracking 取現值再整份回傳）。頁面層級追蹤放在 upsert_content 的 design.settings.tracking 或 upsert_sales_page 的 doc.tracking。',
