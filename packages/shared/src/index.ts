...
export const SETTING_KEYS = {
  ...
  /** 主管理員 Email＝第一位管理員建立時輸入的 Email（永久可註冊／收驗證碼；改 Email 會跟著改） */
  adminPrimaryEmail: 'admin.primaryEmail',
  ...
  trackingOpenaiPixel: 'tracking.openaiPixel',
  ...
  trackingScroll: 'tracking.scroll',
...
export const OPS_ACTIONS = {
  ...
  list_mail_templates: { desc: '列出信件範本（13 種通知：主旨／內文／變數／是否自訂／自動回覆是否啟用）', mutating: false },
  ...
  update_admin: { desc: '修改管理員（idOrEmail；email 換 Email（主管理員 Email 與 mail.adminTo 會跟著改）／password／displayName／role／status active|suspended）', mutating: true },
  ...
  set_tracking: { desc: '設定網站層級追蹤碼（套用到所有頁面；scroll{enabled,percents[],event} 頁面滑動深度事件；ga4、gtm、fbPixel、tiktok、lineTag、openaiPixel（ChatGPT Ads）、googleAdsId、googleAdsLabel、head、bodyTop、bodyBottom、events{pageView,viewContent,addToCart,initiateCheckout,purchase}）', mutating: true },
